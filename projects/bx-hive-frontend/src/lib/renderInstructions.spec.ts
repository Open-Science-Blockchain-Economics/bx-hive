...
describe('trustVariationTokens', () => {
  it('renders ALGO amounts in whole units', () => {
    const tokens = trustVariationTokens(config(), ALGO, DEFAULT_ROLE_LABELS)
  ...
  it('renders a non-ALGO asset in whole units, not base units', () => {
    ...
    const tokens = trustVariationTokens(config({ assetId: 31_566_704n }), USDC, DEFAULT_ROLE_LABELS)
    expect(tokens.e1).toBe('100 USDC')
    expect(tokens.unit).toBe('0.1 USDC')
  })

  it('renders an indivisible asset without a decimal point', () => {
    const tokens = trustVariationTokens(config({ e1: 5n, assetId: 700n }), TICKET, DEFAULT_ROLE_LABELS)
    expect(tokens.e1).toBe('5 TICKET')
  })

  it('strips trailing zeros from the fractional part', () => {
    const tokens = trustVariationTokens(config({ e1: 1_500_000n }), ALGO, DEFAULT_ROLE_LABELS)
    expect(tokens.e1).toBe('1.5 ALGO')
  })

  it('renders the multiplier as a bare number with no unit', () => {
    const tokens = trustVariationTokens(config({ multiplier: 3n }), USDC, DEFAULT_ROLE_LABELS)
    expect(tokens.multiplier).toBe('3')
  })

  it('renders every monetary token, including a zero endowment', () => {
    const tokens = trustVariationTokens(config({ e2: 0n }), USDC, DEFAULT_ROLE_LABELS)
    expect(tokens.e2).toBe('0 USDC')
  })

  it('omits the suffix for an asset with no unit name, leaving no trailing space', () => {
    const tokens = trustVariationTokens(config(), UNNAMED, DEFAULT_ROLE_LABELS)
    expect(tokens.e1).toBe('100')
    expect(tokens.unit).toBe('0.1')
  })

  it("supplies the experimenter's role labels", () => {
    const tokens = trustVariationTokens(config(), USDC, { investorLabel: 'DM1', trusteeLabel: 'DM2' })
    expect(tokens.investorLabel).toBe('DM1')
    expect(tokens.trusteeLabel).toBe('DM2')
  })
})

describe('renderInstructions', () => {
  it('substitutes known tokens', () => {
    const tokens = trustVariationTokens(config(), USDC, DEFAULT_ROLE_LABELS)
  ...
  it('substitutes the role labels into the shared instructions', () => {
    const tokens = trustVariationTokens(config(), USDC, { investorLabel: 'DM1', trusteeLabel: 'DM2' })
    expect(renderInstructions('{{investorLabel}} sends to {{trusteeLabel}}.', tokens)).toBe('DM1 sends to DM2.')
  })
})
