...
export function variationTooltip(v: VariationInfo, config: VariationConfig | undefined, decimals: number): string {
  const status = config ? statusLabel(config) : 'Loading\u2026'
  if (!config) return `${v.label} \u00b7 ${status}`
  const e1 = baseUnitsToWhole(config.e1, decimals).toFixed(3)
  const e2 = baseUnitsToWhole(config.e2, decimals).toFixed(3)
  const unit = baseUnitsToWhole(config.unit, decimals).toFixed(3)
