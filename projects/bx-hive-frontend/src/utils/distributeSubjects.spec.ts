...
import { pickVariationRoundRobin, type VariationSlot } from './distributeSubjects'

const s = (appId: bigint, subjectCount: number, maxSubjects: number): VariationSlot => ({
  appId,
  subjectCount,
  maxSubjects,
})
...
describe('pickVariationRoundRobin', () => {
  ...
  it('treats maxSubjects=0 as unlimited even when subjectCount is large', () => {
    expect(pickVariationRoundRobin([s(1n, 1000, 0)])).toBe(1n)
  })

  it('filters out full capped slots and picks from the rest', () => {
    // var 1 is full; var 2 is partial. Var 2 has even count → fewest-subjects branch.
    expect(pickVariationRoundRobin([s(1n, 2, 2), s(2n, 4, 10)])).toBe(2n)
  })

  it('prioritises a variation with an odd (incomplete-pair) subject count', () => {
    // var 1 has 0 subjects, var 2 has 1 (odd). Picker should pair var 2 first.
    expect(pickVariationRoundRobin([s(1n, 0, 0), s(2n, 1, 0)])).toBe(2n)
  })

  it('among multiple incomplete-pair candidates, picks the one with fewest subjects', () => {
    expect(pickVariationRoundRobin([s(1n, 5, 0), s(2n, 1, 0), s(3n, 3, 0)])).toBe(2n)
  })

  it('with all even counts and no caps, picks the variation with fewest subjects', () => {
    expect(pickVariationRoundRobin([s(1n, 4, 0), s(2n, 0, 0), s(3n, 2, 0)])).toBe(2n)
  })

  it('does not pick a full slot even when its subjectCount is odd', () => {
  ...
  it('mixes capped and unlimited correctly: unlimited counts as available regardless of count', () => {
    // var 1 full, var 2 unlimited at 6 subjects (even). Unlimited wins by elimination.
    expect(pickVariationRoundRobin([s(1n, 5, 5), s(2n, 6, 0)])).toBe(2n)
  })

  it('breaks ties on fewest-subjects branch by taking the first matching slot', () => {
    // two slots tied at 2 subjects each. reduce starts at index 0, strictly-less comparison
