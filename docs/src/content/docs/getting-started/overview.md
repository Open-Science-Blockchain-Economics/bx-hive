...
Every user on bx-hive registers as either an Experimenter or a Subject. Your role determines what you can do on the platform.
...
- Monitor subject enrollment and match progress
...
Subjects participate in experiments. They:
...
See the [Subject guides](../../subjects/dashboard/) for details on joining and playing.
...
4. **Subjects join** — Enrolled subjects are paired into matches (one Investor, one Trustee per match).
