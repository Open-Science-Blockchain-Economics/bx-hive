...
description: How to manage registration, monitor subjects, and oversee experiment lifecycle.
---
...
- **Close** a variation to prevent new subjects from enrolling. Existing subjects and in-progress matches are not affected.
- **Reopen** a closed variation to accept new subjects again.
...
| **Subjects** | Total enrolled with breakdown: playing (in a match) vs waiting (unmatched) |
...
This is useful during active sessions when subjects are enrolling and matches are progressing in real time.

### Per-variation subjects

Select a variation tab to see its subjects table. Each row shows:

- **Address** — The subject's wallet address (truncated for readability)
- **Status** — Either "Assigned" (paired in a match) or "Waiting" (enrolled but not yet matched)

> **[Screenshot: Overview strip and subjects table with auto-refresh enabled]**
...
1. Select an **Investor** from the dropdown of unassigned subjects.
2. Select a **Trustee** from the dropdown of unassigned subjects.
3. Click **Create Match**.

The two subjects cannot be the same address. At least 2 unassigned subjects must be available for the match creation form to appear.

### Auto-match

Toggle **Auto Match** to automatically pair unassigned subjects on a first-in, first-out (FIFO) basis. When enabled, a pulsing green indicator shows that auto-matching is active. As new subjects enroll and become available, they are paired automatically without manual intervention.
...
1. **Active** — At least one variation is open for registration. Subjects can enroll and matches can be created.
2. **Closed** — All variations are closed. No new subjects can enroll, but existing matches continue to play out.
